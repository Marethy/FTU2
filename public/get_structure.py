...
def build_tree(dir_path, prefix=''):
    ...
    entries = [e for e in entries if e not in {'node_modules', '.git', '__pycache__', 'bin', 'obj', 'structure.md', 'get_structure.py', '.mvn', '.vscode', 'target', 'test', '.env', '.gitattributes', '.gitignore', 'Dockerfile', 'HELP.md', 'mvnw', 'mvnw.cmd', 'data'}]
    lines = []
